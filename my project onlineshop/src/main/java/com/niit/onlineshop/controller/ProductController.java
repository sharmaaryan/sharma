...
*/	
	...
	@RequestMapping(value="productget-{p_id}")
	public String getSelectedProduct(@PathVariable("p_id") String p_id,Model model,RedirectAttributes redirectAttributes)
	{
		model.addAttribute("selectedProduct",this.productDAO.get(p_id));
		model.addAttribute("cname",productDAO.get(p_id).getCategory().getName());
		model.addAttribute("sname",productDAO.get(p_id).getSupplier().getName());
	...
	public String prodlist(@PathVariable("id") String id,Model model)
	{
		System.out.println("Category id is "+id);
		model.addAttribute("plist",this.productDAO.getByCatPro(id));
